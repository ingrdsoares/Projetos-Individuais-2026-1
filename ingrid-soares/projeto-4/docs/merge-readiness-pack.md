...
1. **Camada de Extração de Dados:** 
   ...
   - PDFParser utilizando PyMuPDF para conversão de PDF para Markdown (estratégia de Full-Scan e Chunking Semântico).
...
3. **Catálogo de Dados e Linhagem:**
   - Banco de dados SQLite rastreando a linhagem exata: URL original -> Hash do Arquivo -> ID do Documento -> Métricas Extraídas.

## Testes e Validação
- [x] **Idempotência:** Testada via hash SHA-256, evitando reprocessamento de arquivos já existentes.
...
- [x] **Testes Unitários:** Validação de core e banco de dados via pytest.

## Checklist de Revisão Final
- [x] Pipeline executa do início ao fim (Ingestão -> Processamento -> API).
...
- [x] Otimização de tokens via Chunking Semântico implementada.
